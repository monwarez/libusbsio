...
static int32_t get_max_out_report_length(IOHIDDeviceRef device)
{
...
hid_device * HID_API_EXPORT hid_open_path(const char *path)
{
	...
	if (ret == kIOReturnSuccess) {
		...
		dev->max_output_report_len = (CFIndex)get_max_out_report_length(dev->device_handle);
		dev->usage_page = get_int_property(dev->device_handle, CFSTR(kIOHIDPrimaryUsagePageKey));
		dev->usage = get_int_property(dev->device_handle, CFSTR(kIOHIDPrimaryUsageKey));
...
int HID_API_EXPORT hid_get_report_lengths(hid_device* device, unsigned short* output_report_length, unsigned short* input_report_length)
{
    if (output_report_length)
        *output_report_length = (unsigned short)device->max_output_report_len;
    if (input_report_length)
        *input_report_length = (unsigned short)device->max_input_report_len;
    return 0;
}
