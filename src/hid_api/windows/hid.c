...
struct hid_device_info HID_API_EXPORT *HID_API_CALL hid_enumerate(unsigned short vendor_id, unsigned short product_id)
{
    ...
    for (;;)
    {
        ...
        if ((vendor_id == 0x0 || attrib.VendorID == vendor_id) && (product_id == 0x0 || attrib.ProductID == product_id))
        {
            ...
            cur_dev->serial_number = _wcsdup(res ? wstr : L"");

            /* Manufacturer String */
            res = HidD_GetManufacturerString(write_handle, wstr, sizeof(wstr));
            wstr[WSTR_LEN - 1] = 0x0000;
            cur_dev->manufacturer_string = _wcsdup(res ? wstr : L"");

            /* Product String */
            res = HidD_GetProductString(write_handle, wstr, sizeof(wstr));
            wstr[WSTR_LEN - 1] = 0x0000;
            cur_dev->product_string = _wcsdup(res ? wstr : L"");
...
int HID_API_EXPORT HID_API_CALL hid_read_timeout(hid_device *dev, unsigned char *data, size_t length, int milliseconds)
{
    ...
    if (!dev->read_pending)
    {
        ...
        res = ReadFile(dev->device_handle, dev->read_buf, (DWORD)dev->input_report_length, &bytes_read, &dev->read_ol);
...
int HID_API_EXPORT HID_API_CALL hid_get_feature_report(hid_device *dev, unsigned char *data, size_t length)
{
    ...
    res = DeviceIoControl(dev->device_handle, IOCTL_HID_GET_FEATURE, data, (DWORD)length, data, (DWORD)length, &bytes_returned, &ol);
...
int HID_API_EXPORT hid_get_report_lengths(hid_device* device, unsigned short* output_report_length, unsigned short* input_report_length)
{
    ...
    if (input_report_length)
        *input_report_length = (unsigned short)device->input_report_length;
    return 0;
} 
